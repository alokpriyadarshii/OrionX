// ============================================
// OrionX UI — Main Application Orchestrator
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__orionxDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'OrionX Chat';
  ...
  if (_opener) window._orionxRouteOpener = _opener;
  ...
  fetch(`${API_BASE}/api/auth/status`, { credentials: 'same-origin' })
    ...
    .then(d => {
      ...
      if (userBarName && (d.username || d.is_admin)) {
        let displayName = d.username;
        if (!displayName && d.is_admin) displayName = 'admin';
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('orionx-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'orionx-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message OrionX...');
      }
  ...
  const UI_VIS_KEY = 'orionx-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'orionx-toolbar-visibility';
    try {
...
// ============================================
function startOrionXApp() {
  if (window.__orionxAppStarted) return;
  window.__orionxAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('orionx-doc-open-' + _curSession) === '1') {
  ...
  window._orionxBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._orionxRouteOpener) {
          try { window._orionxRouteOpener(); } catch (_) {}
          window._orionxRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startOrionXApp, { once: true });
} else {
  startOrionXApp();
}
