...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__orionxActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'orionx-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'orionx-doc-minimized-' + sessionId;
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__orionxActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('orionx-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('orionx-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'orionx-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('orionx-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('orionx-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('orionx-suggestions-' + docId);
      if (!raw) return;
