...
let _sortMode = Storage.get('orionx-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'OrionX Chat';
...
const FOLDER_STATE_KEY = 'orionx-folder-state';
const FOLDER_ORDER_KEY = 'orionx-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to OrionX kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'OrionX Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('orionx-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('orionx-session-sort', mode);
  else Storage.remove('orionx-session-sort');
  renderSessionList();
}
