...
try:
    ...
    # OrionX is single-process by convention, so this is acceptable, but be aware that
...
if AUTH_ENABLED:
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. OrionX's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-OrionX-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-OrionX-Owner") or "").strip()
...
async def _startup_event():
    ...
    # `ORIONX_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("ORIONX_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (ORIONX_INPROCESS_TASKS=0); "
