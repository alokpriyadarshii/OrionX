...
def _apply_orionx_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-OrionX-Origin"] = ORIONX_MAIL_ORIGIN
    if kind:
        msg["X-OrionX-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-OrionX-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def _normalize_addr_field(field: str) -> str:
    ...
    trailing comma (e.g. `alokpriyadarshi618@gmail.com,`) and most MTAs reject the
    resulting `To: alokpriyadarshi618@gmail.com,` line as a syntax error. Collapse
...
def setup_email_routes():
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the OrionX marker header, but include the subject
                # fallback too. The fallback uses a distinct OrionX prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-OrionX-Kind "reminder" SUBJECT "Reminder (OrionX):"{from_clause})',
                )
    ...
    @router.delete("/orionx/reminders")
    async def delete_orionx_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as OrionX reminders."""
        if account_id:
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-OrionX-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (OrionX):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (OrionX):")})'))
                            # Legacy reminders created before the OrionX
            ...
            logger.error(f"delete_orionx_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", orionx_kind=None, orionx_ref=None,
    ):
        ...
        _apply_orionx_headers(outer, orionx_kind or "scheduled", orionx_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, orionx_kind, owner)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("orionx_kind") or "scheduled",
                owner or "",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="orionx.local")
        ...
        if req.orionx_kind:
            _apply_orionx_headers(outer, req.orionx_kind)
