                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in OrionX (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in OrionX" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "orionx_kind" if "orionx_kind" in cols else "'scheduled' AS orionx_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                orionx_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-OrionX-Origin"] = "orionx-ui"
                outer["X-OrionX-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", orionx_kind or "scheduled")[:64]
                outer["X-OrionX-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `orionx-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `ORIONX_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `orionx-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("ORIONX_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `ORIONX_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (ORIONX_INPROCESS_POLLERS=0); "
            "drive `orionx-mail poll-scheduled` externally."
        )
        return
    import asyncio
