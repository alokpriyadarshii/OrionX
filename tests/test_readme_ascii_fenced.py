...
def test_readme_opens_with_wordmark_title():
    # The README must still open with a recognizable OrionX title. Some
    # distributions use the wordmark image; this trimmed one keeps only README
    # markdown and opens with the project H1.
    head = "\n".join(README.read_text(encoding="utf-8").splitlines()[:15])
    assert (
        'alt="OrionX"' in head or "# OrionX" in head
    ), "README must open with the OrionX identity"
