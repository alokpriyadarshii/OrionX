"""Regression: `orionx-calendar list` must select events that OVERLAP the
...
`orionx-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "orionx-calendar"
    loader = importlib.machinery.SourceFileLoader("orionx_calendar_cli", str(path))
