...
def _compose_env_names(path: Path) -> set[str]:
    ...
    env = compose["services"]["orionx"]["environment"]
...
def _upload_limit_env_names() -> set[str]:
    ...
    return set(re.findall(r'"(ORIONX_[A-Z_]*BYTES)"', source)) | {
        "ORIONX_CHAT_UPLOAD_MAX_BYTES"
    }
...
def test_testing_docs_use_project_venv_for_python_validation():
    existing_docs = [path for path in TEST_DOCS if path.exists()]
    if not existing_docs:
        pytest.skip("testing docs were intentionally removed from this distribution")
