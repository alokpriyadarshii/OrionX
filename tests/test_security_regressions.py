...
def test_readme_native_quickstart_uses_loopback():
    ...
    if setup_doc.exists():
        docs += "\n" + setup_doc.read_text(encoding="utf-8")
    assert "python -m uvicorn app:app --host 127.0.0.1 --port 7000" in docs
    assert "APP_BIND=127.0.0.1" in docs
...
def test_ollama_cookbook_runner_does_not_force_public_bind():
    ...
    assert 'OLLAMA_HOST="0.0.0.0:${ORIONX_OLLAMA_PORT}" ollama serve' not in route
    assert 'OLLAMA_HOST="${ORIONX_OLLAMA_HOST}:${ORIONX_OLLAMA_PORT}" ollama serve' in route
...
def test_inprocess_pollers_gate(monkeypatch):
    """The ORIONX_INPROCESS_POLLERS env var must let operators kill
    the asyncio pollers when cron / systemd is driving the one-shot
    `orionx-mail poll-*` CLI subcommands instead. Two pollers racing
    ...
    for on in ("1", "true", "yes", "anything-truthy"):
        monkeypatch.setenv("ORIONX_INPROCESS_POLLERS", on)
...
def _load_search_content_for_test(monkeypatch, name="services.search.content_under_test"):
    ...
    cache.CONTENT_CACHE_DIR = Path("/tmp/orionx-test-content-cache")
...
def test_google_mcp_oauth_uses_configured_redirect_base(monkeypatch):
    monkeypatch.setenv("OAUTH_REDIRECT_BASE_URL", "https://orionx.example/app/")
    ...
    assert (
        ...
        == "https://orionx.example/app/api/mcp/oauth/callback"
    )
